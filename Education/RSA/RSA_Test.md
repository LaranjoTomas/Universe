...
**R:** To minimize packet loss and its impact on services operating over UDP in a drone network within a controlled space with a low probability of complete disconnection, a highly effective solution would be to implement the QUIC protocol. This protocol is specifically designed to address the limitations of traditional transport protocols like TCP in challenging wireless and ad-hoc environments, which closely resemble the conditions of a drone network. **It's built for UDP**, QUIC packets are carried with UDP datagrams, avoids head-of-line blocking across multiple streams. With QUIC, when a packet loss occurs, only the streams with data in that **specific packet are blocked waiting for retransmission**, **while other streams can continue making progress**. **RTT-Independent Congestion Control**, Fast Handshake. In summary, QUIC provides a robust transport layer solution that mitigates the impact of packet loss through its multi-stream architecture, intelligent loss recovery, RTT-independent congestion control, and efficient handshake. While QUIC doesn't prevent packets from being physically lost in the air, it ensures that such losses are handled far more gracefully and efficiently than with raw UDP or standard TCP, thereby minimizing the effective impact of packet loss on application performance and user experience.
...
**R:** 
Aqui CPMs seriam impossíveis, pois não existiria a informação dos sensores neste caso.
